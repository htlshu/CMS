...
var storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    req.body.productsImg = '/uploads/logos/' + _filename
    cb(null, _filename)
  }
})
...
var upload = multer({
  storage,
  fileFilter
}).single('productsImg') // .single处理单文件上层


// 在upload中间件外面套上一个空壳中间件，目的是为了让upload处理错误后选择是否继续向下执行
const fileUpload_products = function (req, res, next) {
...
module.exports = fileUpload_products